...
def get_python_submodules(moose_core_path):
    """Discover Python subpackages in moose-core by scanning the source tree."""
    moose_py_dir = os.path.join(moose_core_path, 'python', 'moose')
    if not os.path.isdir(moose_py_dir):
        return []
    submodules = []
    for item in sorted(os.listdir(moose_py_dir)):
        item_path = os.path.join(moose_py_dir, item)
        init = os.path.join(item_path, '__init__.py')
        if os.path.isdir(item_path) and not item.startswith('_') and os.path.isfile(init):
            submodules.append(item)
    return submodules


def generate_submodule_rst(submodule_name, out_dir):
    """Generate an autodoc RST file for a moose Python subpackage."""
    full_name = f'moose.{submodule_name}'
    out_path = os.path.join(out_dir, f'moose_{submodule_name}.rst')
    if os.path.isfile(out_path):
        return f'moose_{submodule_name}'
    lines = [
        full_name,
        '=' * len(full_name),
        '',
...
def generate():
    ...
    with open(index_path, 'w') as f:
        f.write('.. NOTE: Auto-generated by generate_references.py — do not edit manually.\n\n')
        f.write('References\n')
        f.write('==========\n\n')
